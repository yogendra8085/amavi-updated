...
class PlaceholderTermsCondition extends StatefulWidget {
  final String strHeading;

  PlaceholderTermsCondition(this.strHeading);

  @override
  _PlaceholderTermsConditionWidgetState createState() =>
      _PlaceholderTermsConditionWidgetState();
}

class _PlaceholderTermsConditionWidgetState
    extends State<PlaceholderTermsCondition> {
  bool isLoading = true;
  String ?strToken;
  String ?_description;
  String _headerTitle = "";
...
//      ),
  ...
  Future<void> getTermsConditionData() async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      ...
      String strBaseUrl = Constants.TERMS_AND_CONDITION_URL + (strToken ?? "");
log("Url ==>  $strBaseUrl");
      final response =
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData["description"] != null) {
          ...
          _headerTitle = jsonData["heading_title"].toString().replaceAll("&amp;", "&").replaceAll("&quot;", '"');
